...
function getRequest(url: string): Promise<{status: number, body: string}> {
...
export class Server {
  ...
  constructor(public config: config.Config) {
    ...
    // var corsOptions = {
    //   origin: function (origin:string, callback: (e:Error | null, b?:boolean)
    //   => void) {
    ...
    this.app.get('/api/job_quality', async (_req, res) => {
      try {
        let url = `${this.config.apiServerUrl}/client_jobs/` +
  ...
  public async postAnswer(
      clientJobKey: string, answerBody: {[k: string]: string}):
    ...
    try {
      ...
      let result = await new Promise<{status: number, body: string}>(
          (resolve, reject) => {
            request(
                {method: 'POST', uri: url, json: {answer: answerBody}},
  ...
  stop(): Promise<void> {
    return new Promise<void>(
        (resolve: () => void, _: (impossible_error?: Error) => void) => {
