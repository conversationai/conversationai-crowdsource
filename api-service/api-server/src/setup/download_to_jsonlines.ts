...
interface Params {
  file: string, client_key_regexp: string, gcloud_project_id: string,
      spanner_instance: string, spanner_db: string, query: string,
      writeflags: string,
}
;
...
async function queryAndApply<T>(
    spannerDatabase: spanner.Database, query: string,
    f: (row: T) => Promise<void>): Promise<void> {
  let streamingQuery: spanner.StreamingQuery =
      spannerDatabase.runStream({sql: query});
  ...
  let complete = new Promise<void>((resolve, reject) => {
    ...
    streamingQuery.on('data', (row) => {
      ...
      if (resultCount % 100 === 0) {
        console.log(
            `Next 100. (queueSize: ${queueSize}; resultCount: ${resultCount})`);
      }
    ...
    streamingQuery.on('error', (e) => {
      reject(e);
    });
    streamingQuery.on('end', () => {
      console.log(
          `Stream End. (queueSize: ${queueSize}; resultCount: ${resultCount})`);
      resolve();
    });
  })
...
let args =
    yargs.option('file', {describe: 'Path to JSON file of questions to upload'})
        .option('query', {describe: 'SQL query to send.'})
        .option('writeflags', {
          describe:
              'if `w`, overwrite the output file even if it already exists, ' +
        ...
        .demandOption(
            [
              'file', 'query', 'gcloud_project_id', 'spanner_instance',
              'spanner_db'
            ],
