...
interface Params {
  infile: string, output_path_prefix: string, output_path_postfix: string,
      left_id_prefix: string, right_id_prefix: string, left_text_prefix: string,
      right_text_prefix: string, entries_per_row: number, rows_per_file: number,
}
;
...
function createAllPairsFiles(data: InputObj[]): OutFileData[] {
  ...
  for (let d1 of data) {
    for (let d2 of data) {
      ...
      out_pairs_for_row[`${args.left_text_prefix}${pair_id_postfix}`] =
          d1.comment_text;
      out_pairs_for_row[`${args.right_text_prefix}${pair_id_postfix}`] =
          d2.comment_text;
  ...
  if (pair_id > 1) {
    console.warn(
        `Extra pairs not emitted: ${JSON.stringify(out_pairs_for_row)}`);
  }
...
async function main(args: Params) {
  ...
  let readLineEventHandler = readline.createInterface(instream);
  ...
  readLineEventHandler.on('line', async function(line) {
    let obj: InputObj = JSON.parse(line);
    lineCount++;
    data.push(obj);
  });

  readLineEventHandler.on('close', function() {
    let outFiles = createAllPairsFiles(data)

    // Output everything.
    let outfile_id = 1;
    for (let outfile of out_files) {
      let outstream = fs.createWriteStream(
          `${args.output_path_prefix}_${outfile_id}_of_${out_files.length}${
...
let args =
    yargs
        ...
        .option('left_text_prefix', {
          default: 'i.comment_a',
          describe:
              'Prefix string for the comment text property of the left example'
        })
        ...
        .option(
            'entries_per_row',
            {default: 5, describe: 'The number of pairs to bundle per row.'})
        ...
        .option(
            'output_path_postfix',
            {default: '.jsonl', describe: 'The postfix for output files'})
        .demandOption(
